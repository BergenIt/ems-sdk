...
Данная операция отвечает за получение данных по температуре устройства по протоколу SNMP. Выполняется после получения сервисом следующего GRPC-запроса.
...
* `metric_templates`:
  ...
  * Описание: Массив шаблонов системных метрик.
