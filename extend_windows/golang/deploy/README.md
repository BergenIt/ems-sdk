...
Для возможности перегрузки операции в Dockerfile необходимо указать один из следующих лейблов:
 - **`ems.windows.memory.operation-system`** для перегрузки операции по определенной операционной системе устройства или названием и версией операционной системы устройства (разделенных пробелом)
 - **`ems.windows.memory`** для перегурузки операции без привязке к ОС
